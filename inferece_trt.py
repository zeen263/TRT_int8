...
def load_test_case(test_images, pagelocked_buffer, preprocess_func):
    # Expected input dimensions
    C, H, W = (3, 608, 608)
    # Normalize the images, concatenate them and copy to pagelocked memory.
    data = np.asarray([preprocess_func(PIL.Image.open(img), C, H, W) for img in test_images]).flatten()
...
def infer(engine_path, preprocess_func, batch_size=8, input_images=[], labels=[], top_n_classes=3):
    with open(engine_path, 'rb') as f, trt.Runtime(trt.Logger(trt.Logger.WARNING)) as runtime:
        ...
        with engine.create_execution_context() as context:
            ...
            """
            ARGOS/LP 모델 출력 파싱할 수 있게 고치기 (simplebbox였던 것 참조)
            """
            out = outputs[0]
            ...
            for test_image, batch_out in zip(test_images, batch_outs):
                topk_indices = np.argsort(batch_out)[-1*top_n_classes:][::-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--test_file', default=None, type=str,
                        help="Path to input image.")
    parser.add_argument('-d', '--test_dir', default=None, type=str,
    ...
    parser.add_argument('-n', '--top_n_classes', default=3, type=int,
                        help="Top-N predictions to output.")
    ...
    input_images = get_inputs(args.test_file, args.test_dir)
    ...
    infer(args.engine, preprocess_func, batch_size=args.batch_size, input_images=input_images,
          labels=labels, top_n_classes=args.top_n_classes)
